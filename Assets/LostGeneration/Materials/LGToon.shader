...
Shader "Lost Generation/LGToon" {
    Properties {
        ...
        [PerRendererData] _UVRectOffset ("UV Sub-Rectangle Offset", Vector) = (0, 0.3, 0, 1)
        [PerRendererData] _UVRectSize ("UV Sub-Rectangle Size", Vector) = (1, 0.3, 0, 1)
    }
    SubShader {
        Tags {
            "RenderType"="Opaque"
        ...
        Pass {
            ...
            uniform float4 _UVRectOffset;
            uniform float4 _UVRectSize;
            struct VertexInput {
                float4 vertex : POSITION;
                float3 normal : NORMAL;
            ...
            VertexOutput vert (VertexInput v) {
                VertexOutput o = (VertexOutput)0;
                o.uv0 = _UVRectOffset.xy + (v.texcoord0 * _UVRectSize.xy);
                o.vertexColor = v.vertexColor;
...
////// Emissive:
        ...
        Pass {
            ...
            uniform float4 _Flare;
            uniform sampler2D _DiffuseTexture; uniform float4 _DiffuseTexture_ST;
            uniform float2 _UVRectOffset;
            uniform float2 _UVRectSize;
            struct VertexInput {
                float4 vertex : POSITION;
                float3 normal : NORMAL;
                float2 texcoord0 : TEXCOORD0;
