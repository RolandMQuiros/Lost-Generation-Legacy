    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float3 ambientTint = (ambient*tinting);

                float nDotL = dot(lightDirection,i.normalDir);
                float3 node_2495 = (tinting*round(nDotL)*_LightColor0.rgb);
                return fixed4((ambientTint*0.6*node_2495),0);
            }
            ENDCG
        }
        Pass {
...
////// Emissive:
                ...
                float3 ambientTint = (ambient*tinting);
                float3 emissive = ambientTint;
                float nDotL = dot(lightDirection,i.normalDir);
                float3 node_2495 = (tinting*round(nDotL)*(_LightColor0.rgb*attenuation));
